        ...
        <flux:main>
            ...
            <nav aria-label="Breadcrumb" class="mb-4">
                <ol class="flex items-center gap-1.5 text-sm text-zinc-500 dark:text-zinc-400">
                    <li>
                        <a href="{{ route('dashboard') }}" wire:navigate class="hover:text-zinc-900 dark:hover:text-white transition">
                            <flux:icon icon="home" variant="micro" />
                        </a>
                    </li>
                    @foreach($breadcrumb as $i => $crumb)
                        <li>
                            <flux:icon icon="chevron-right" variant="micro" class="text-zinc-300 dark:text-zinc-600" />
                        </li>
                        <li @if($loop->last) aria-current="page" @endif>
                            @if(is_array($crumb) && isset($crumb['url']))
                                <a href="{{ $crumb['url'] }}" wire:navigate class="hover:text-zinc-900 dark:hover:text-white transition">{{ $crumb['label'] }}</a>
                            @else
                                <span class="font-medium text-zinc-900 dark:text-white">{{ is_array($crumb) ? $crumb['label'] : $crumb }}</span>
                            @endif
                        </li>
                    @endforeach
                </ol>
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ $pageDescription }}</p>
                @endisset
            </div>
            @endisset
