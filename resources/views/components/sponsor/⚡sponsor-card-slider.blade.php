...
new class extends Component
{
    ...
    public function submit()
    {
        $this->validate([
            ...
            'mediaFile.required' => 'Wajib memilih file visual Card Slider (gambar atau video)!',
            'mediaFile.max' => 'Ukuran file visual maksimal 20 MB!',
            'thumbnailFile.max' => 'Ukuran thumbnail maksimal 5 MB!',
        ]);

        $mediaPath = 'assets/images/PHOTO-2026-07-22-20-20-24.jpg';
        if ($this->mediaFile) {
            $mediaPath = $this->mediaFile->store('campaigns/card', 'public');
        ...
        $this->successMessage = 'Pengajuan promo Card Slider berhasil dikirim! Menunggu moderasi Super Admin.';
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-5">
        <div>
            <div class="flex items-center gap-2">
                <h1 class="text-2xl font-bold text-white tracking-tight">Pengajuan Card Slider Beranda</h1>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold bg-teal-900/60 text-teal-300 border border-teal-700/50">
                    Mosaic Promo Grid Beranda (Card)
                </span>
            </div>
            <p class="text-xs text-zinc-400 mt-1">
                Tampilan card slider vertikal/square di dalam susunan grid mosaic beranda aplikasi.
            </p>
        </div>

        <button type="button" wire:click="openCreateModal" class="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold shadow-lg shadow-teal-600/30 transition flex items-center gap-2">
            <flux:icon name="plus" class="w-4 h-4" />
            <span>Ajukan Card Slider Baru</span>
        </button>
    </div>
    ...
    <!-- Panduan Spesifikasi Card Slider (Dark Styled) -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-2xl bg-zinc-900/90 border border-zinc-800 text-xs">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-start gap-3 min-w-0">
                <div class="p-2 rounded-lg bg-teal-950/60 text-teal-400 border border-teal-900/50 shrink-0">
                    <flux:icon name="squares-2x2" class="w-4 h-4" />
                </div>
                <div>
                    <span class="font-bold text-white block">Ukuran Card Portrait (4:5 / 1:1)</span>
                    <span class="text-zinc-400 text-[11px] leading-tight block mt-0.5">Rekomendasi 800 × 1000 px atau 800 × 800 px (JPG, PNG, WEBP).</span>
                </div>
            </div>
            <button type="button" 
                wire:click="$set('showPreviewModal', true)" 
                class="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-teal-600/20 hover:bg-teal-600/30 text-teal-400 hover:text-teal-300 border border-teal-500/30 font-semibold text-xs transition duration-150 shadow-sm cursor-pointer active:scale-95">
        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                <span class="font-bold text-white block">Posisi Grid Mosaic Beranda</span>
                <span class="text-zinc-400">Menyatu secara mulus dengan card video pembelajaran dan rekomendasi alat.</span>
            </div>
        </div>
        <div class="flex items-start gap-3">
            ...
            <div>
                <span class="font-bold text-white block">Gambar atau Video Demo (Maks 20 MB)</span>
                <span class="text-zinc-400">Dukungan video pendek demonstrasi produk auto-looping tanpa suara.</span>
            </div>
        </div>
    </div>

    <!-- Tabel Pengajuan Card Slider -->
    <div class="bg-zinc-900/90 rounded-2xl border border-zinc-800 p-5 shadow-sm">
        <h3 class="font-bold text-base text-white mb-4">Riwayat Pengajuan Card Slider</h3>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-zinc-800 text-xs font-semibold text-zinc-400 uppercase bg-zinc-950/40">
                        ...
                        <th class="py-3 px-3">Judul Card Promo</th>
                ...
                <tbody class="divide-y divide-zinc-800/80">
                    ...
                    <tr class="hover:bg-zinc-800/50 transition">
                        <td class="py-3 px-3" style="width: 100px; min-width: 100px; max-width: 100px;">
                            <div class="rounded-xl bg-zinc-950 overflow-hidden relative border border-zinc-800 flex items-center justify-center shadow-xs group" style="width: 72px; height: 80px;">
                                <img src="{{ $camp->thumbnail_url }}" style="width: 72px; height: 80px; object-fit: cover;" alt="{{ $camp->title }}" onerror="this.src='{{ asset('assets/images/PHOTO-2026-07-22-20-20-24.jpg') }}'" />
                                @if($camp->media_type === 'video')
                                    <div class="absolute inset-0 bg-black/40 group-hover:bg-black/20 flex items-center justify-center text-white transition">
                                        <div class="w-6 h-6 rounded-full bg-teal-600 text-white flex items-center justify-center shadow-md">
                    ...
                    <tr>
                        <td colspan="5" class="py-8 text-center text-zinc-400 text-sm">
                            Belum ada pengajuan Card Slider. Klik "Ajukan Card Slider Baru" di atas.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
        <div class="bg-zinc-900 rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-zinc-800 space-y-4 max-h-[90vh] overflow-y-auto text-zinc-200">
            <div class="flex items-center justify-between border-b border-zinc-800 pb-3">
                <h3 class="font-bold text-lg text-white">Formulir Pengajuan Card Slider Beranda</h3>
            ...
            <form wire:submit.prevent="submit" class="space-y-4">
                <flux:input label="Judul Card Promo" wire:model="title" placeholder="Contoh: LCD Premium Braderparts OLED High Brightness" required />
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <button type="button" wire:click="$set('media_type', 'image')" class="p-2.5 rounded-xl border text-xs font-semibold flex items-center justify-center gap-2 {{ $media_type === 'image' ? 'border-teal-500 bg-teal-950/50 text-teal-300' : 'border-zinc-700 bg-zinc-800 text-zinc-400' }}">
                            <flux:icon name="photo" class="w-4 h-4" /> Gambar Card (Portrait / 1:1)
                        </button>
                        <button type="button" wire:click="$set('media_type', 'video')" class="p-2.5 rounded-xl border text-xs font-semibold flex items-center justify-center gap-2 {{ $media_type === 'video' ? 'border-teal-500 bg-teal-950/50 text-teal-300' : 'border-zinc-700 bg-zinc-800 text-zinc-400' }}">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="block text-xs font-semibold text-zinc-300">Upload File Visual Card (Maks 20 MB)</label>
                        <button type="button" wire:click="$set('showPreviewModal', true)" class="text-[11px] text-teal-400 hover:text-teal-300 flex items-center gap-1 font-medium transition cursor-pointer">
                            <flux:icon name="eye" class="w-3.5 h-3.5" />
                    ...
                    <div wire:loading wire:target="mediaFile" class="text-xs text-teal-400 mt-1 flex items-center gap-1">
                ...
                <flux:textarea label="Catatan / Deskripsi Singkat" wire:model="description" placeholder="Keterangan singkat promo..." rows="2" />
                ...
                <div class="flex items-center justify-end gap-3 pt-3 border-t border-zinc-800">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold transition" wire:loading.attr="disabled">Kirim Pengajuan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <!-- Modal Preview Mockup Tampilan Card Slider / Mosaic -->
    @if($showPreviewModal)
        ...
        <div class="bg-zinc-900 rounded-2xl max-w-lg w-full p-5 shadow-2xl border border-zinc-800 space-y-4 max-h-[92vh] flex flex-col text-zinc-200">
            <div class="flex items-center justify-between border-b border-zinc-800 pb-3 shrink-0">
                <div class="flex items-center gap-2.5">
                    <div class="p-2 rounded-xl bg-teal-950/80 text-teal-400 border border-teal-900/60">
                        <flux:icon name="device-phone-mobile" class="w-4 h-4" />
                    </div>
                    <div>
                        <h3 class="font-bold text-sm text-white">Contoh Tampilan Card Slider / Mosaic</h3>
                        <p class="text-[11px] text-zinc-400">Posisi: Grid Rekomendasi Beranda (4:5 / 1:1)</p>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto pr-1 space-y-3 text-center">
                <div class="rounded-xl overflow-hidden border border-zinc-800 shadow-2xl bg-zinc-950 inline-block max-w-full">
                    <img src="{{ asset('assets/images/previews/preview-card-slider.jpg') }}" 
                         alt="Preview Card Slider di Aplikasi" 
                         class="max-h-[60vh] w-auto mx-auto object-contain">
                </div>
                <div class="p-3 rounded-xl bg-teal-950/30 border border-teal-900/40 text-left text-xs text-teal-300 space-y-1">
                    <div class="font-bold flex items-center gap-1.5 text-teal-400">
                        <flux:icon name="information-circle" class="w-4 h-4 shrink-0" />
                        <span>Panduan Rasio Portrait (4:5 / 1:1 / 800 × 1000 px)</span>
                    </div>
                    <p class="text-[11px] text-zinc-400 leading-relaxed">
                        Area yang dikelilingi <strong class="text-rose-400 font-semibold">kotak merah</strong> di atas menunjukkan letak card promosi Anda pada kolom grid 'Rekomendasi Untukmu'.
                    </p>
                </div>
            </div>
